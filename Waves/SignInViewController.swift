...
class SignInViewController:UIViewController{
    ...
    func setUpWith(viewModel:SignInViewModel, completion: (result: Bool) -> Void){
    ...
    func bindModel(){
        ...
        self.signInViewModel!.userEmailString <~ (self.userEmailTextField?.rac_textSignal()
            .toSignalProducer()
            .map { ($0 as? String) ?? "" }
        ...
        self.signInViewModel!.userPasswordString <~ (self.userPassWordTextField?.rac_textSignal()
            .toSignalProducer()
            .map { ($0 as? String) ?? "" }
            .flatMapError { _ in SignalProducer<String, NoError>.empty })!
        ...
        self.signInViewModel!.hasSentData
            .producer
            .startWithNext({
                [weak self]
                next in
    ...
    func sendInfoToServer(){
        ...
        self.signInViewModel!.submitInformationToServers()
    }
